...
def _patch_json(url: str, payload: dict, bearer: str) -> dict:
    data = json.dumps(payload).encode()
...
def refresh_access_token(refresh_token: str) -> tuple[str, str]:
    ...
    resp = _post_json(AUTH0_TOKEN_URL, {
...
def ensure_access_token(token_file: pathlib.Path) -> str:
    ...
    # Use cached access token if it has more than 60s left
...
def list_contests(access_token: str) -> list[dict]:
    data = _get_json(f"{MPP_API_BASE}/user-contests", access_token)
...
def get_calendar(access_token: str, championship_id: str) -> list[dict]:
    ...
    """
    cal = _get_json(f"{MPP_API_BASE}/championship-calendar/{championship_id}", access_token)

    # Round "1" in the calendar = full group stage, keyed by group letter
    ...
    # Fetch individual match objects to get club IDs
    club_ids: set[str] = set()
    raw_matches: dict[str, dict] = {}
    for mid in all_match_ids:
        m = _get_json(f"{MPP_API_BASE}/championship-match/{mid}", access_token)
        hcid = m["home"]["clubId"]
        acid = m["away"]["clubId"]
        club_ids.add(hcid)
        club_ids.add(acid)
    ...
    # Resolve club names (prefer en-GB, fall back to fr-FR)
    clubs: dict[str, str] = {}
    for cid in club_ids:
        c = _get_json(f"{MPP_API_BASE}/championship-club/{cid}", access_token)
...
def submit_prediction(access_token: str, scope: str, match_id: str,
                      home_score: int, away_score: int) -> dict:
    url = f"{MPP_API_BASE}/user-match-forecasts/entity/{scope}/match/{match_id}"
    return _patch_json(url, {"homeScore": home_score, "awayScore": away_score, "originPage": "home"}, access_token)
...
def find_mpp_match(prediction: dict, mpp_matches: list[dict],
    ...
    for m in mpp_matches:
        mpp_home = m.get("home", "")
        mpp_away = m.get("away", "")
        score = _similarity(pred_home, mpp_home) * 0.5 + _similarity(pred_away, mpp_away) * 0.5
        score_swapped = _similarity(pred_home, mpp_away) * 0.5 + _similarity(pred_away, mpp_home) * 0.5
...
def main():
    ...
    try:
        mpp_matches = get_calendar(access, args.championship_id)
    except urllib.error.HTTPError as e:
        body = e.read().decode(errors="replace")
        sys.exit(f"\nFailed to fetch calendar ({e.code}): {body}")
    print(f"  {len(mpp_matches)} matches loaded")

    # Build mapping: prediction → MPP match
    matched, unmatched = [], []
    for pred in preds:
    ...
    for pred, mpp_m in matched:
        ...
        try:
            submit_prediction(access, args.scope, match_id,
